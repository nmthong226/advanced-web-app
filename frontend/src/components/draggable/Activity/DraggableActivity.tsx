    ...
    <DropdownMenu>
      <div className="flex items-center group">
        ...
        <div
          ref={drag}
          className={cn(
            `rounded-md border px-3 py-2 font-mono truncate text-sm w-full relative ${isDragging ? 'opacity-50' : 'opacity-100'}`,
            backgroundColor,
          )}
          style={{
            cursor: 'grab',
        ...
        >
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <EditEventItemsDialog
              eventCategory={category}
              eventItem={{ id: '', title, backgroundColor, textColor }}
              onEditEventItem={() => {}}
            />
          </Dialog>
          {/* Delete Dialog */}
          ...
          >
            <DialogContent className="left-[10%] justify-center p-2 w-[200px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  <p className="flex flex-wrap text-sm">Delete {title} item?</p>
                </DialogDescription>
              </DialogHeader>
